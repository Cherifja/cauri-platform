    ...
    <div>
      <div className="bg-gradient-to-b from-ink to-ink2">
        <div className="px-5 md:px-8 pt-10 md:pt-16 pb-8 md:pb-14 max-w-6xl mx-auto">
          <p className="text-xs uppercase tracking-[0.2em] mb-2 text-gold">{t("home.tagline")}</p>
          <h1 className="text-3xl md:text-5xl leading-tight mb-3 font-display font-semibold text-cream md:max-w-2xl">
            {t("home.title")}
          </h1>
          <p className="text-sm md:text-base text-sandDeep md:max-w-lg">{t("home.subtitle")}</p>
        </div>
      </div>
      ...
      <div className="px-5 md:px-8 py-6 md:py-12 max-w-6xl mx-auto">
        ...
        {status === "loading" && <p className="text-sm text-ink2">{t("home.loading")}</p>}
        {status === "error" && <p className="text-sm text-clay">{t("home.error")}</p>}
        ...
        {status === "ready" && filteredProperties.length === 0 && (
          <p className="text-sm text-ink2">{t("home.noResults")}</p>
        )}
      ...
      <div className="bg-ink">
        <div className="px-5 md:px-8 py-10 md:py-14 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <p className="text-sm font-medium mb-1 text-cream">{t("home.trust.verified.title")}</p>
              <p className="text-xs text-sandDeep">{t("home.trust.verified.desc")}</p>
            </div>
            <div>
              <p className="text-sm font-medium mb-1 text-cream">{t("home.trust.price.title")}</p>
              <p className="text-xs text-sandDeep">{t("home.trust.price.desc")}</p>
            </div>
            <div>
              <p className="text-sm font-medium mb-1 text-cream">{t("home.trust.support.title")}</p>
              <p className="text-xs text-sandDeep">{t("home.trust.support.desc")}</p>
            </div>
          </div>
        </div>
      </div>
